...
- Automated result: 36 tests passed, 0 failed, 0 skipped.
...
- npx result: a fresh temporary consumer ran `litmo init` and the PASS-to-FAIL `litmo demo` through `npx --yes --package <local-tarball>`.
- Registry result: the official npm registry returned `E404` for `litmo` on 2026-07-14; bare `npx litmo` cannot work until publication.
- Publish result: `npm publish --dry-run` completed as `litmo@0.1.0`; no package was published.
...
| UAT-39 | Run the self-contained `litmo demo` command                | Local evidence passes, signature is changed, mismatch is shown, command exits `0`  | demo-command UAT test              |
...
| UAT-41 | Run npm's official publication dry-run                     | npm accepts the non-private `litmo@0.1.0` package metadata and tarball             | `npm publish --dry-run` checkpoint |
